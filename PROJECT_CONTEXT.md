...
> **Propósito deste arquivo:** contexto completo do projeto para retomada do desenvolvimento em novas sessões. Leia-o integralmente antes de escrever qualquer código. Última atualização: **2026-08-16** (documentação — nota explícita na seção 6/Etapa 12 sobre `cancelar` de impressão também abater estoque de filamento e gerar `custoTotal` quando peso/energia são informados, decisão de negócio intencional achada na auditoria pós-roadmap). Antes disso: **2026-07-20** (**pós-roadmap**: fluxo Pedido→Impressão automatizado + tema escuro moderno no frontend — ver seção 6, "Evolução pós-roadmap"). Antes disso: **2026-07-18, Etapa 19 CONCLUÍDA — roadmap de 19 etapas completo**: upload STL/3MF, rate limiting, aviso de senha default e healthcheck do NGINX corrigido; 104 testes verdes; stack Docker validada em http://localhost.
...
- **`cancelar` também abate estoque e gera custo, de propósito**: os três endpoints (`concluir`/`falhar`/`cancelar`) chamam o mesmo método privado `finalizar()` em `PrintHistoryService`, que sempre executa `consumirFilamento()` e `calcularCusto()` — não há tratamento especial para `PrintStatus.CANCELADA`. Ou seja, se `pesoUtilizadoG`/`consumoEnergiaKwh` forem informados no `PATCH .../cancelar`, o job cancelado abate o mesmo peso do estoque do filamento e recebe `custoTotal` como se tivesse concluído. **Decisão de negócio intencional** (comentário no próprio código-fonte: "material desperdiçado saiu da bobina do mesmo jeito") — uma impressão cancelada no meio do processo já consumiu fisicamente o filamento até aquele ponto, então não faria sentido devolver esse material ao estoque nem zerar o custo. Não é um bug; é o operador quem decide informar ou não peso/energia ao cancelar (se omitir, ambos ficam `null` e nada é abatido/custeado).
...
### Upload de STL/3MF (Etapa 19 — parte 1, decisões)
- **Endpoints** `POST|GET|DELETE /orders/{pedidoId}/itens/{itemId}/arquivo` (`OrderItemFileController`, mesmos perfis do módulo de pedidos). POST é multipart (parte `arquivo`), upsert: substitui o anexo anterior e apaga o antigo do disco. **Anexar/remover segue a regra de edição (só pedido PENDENTE); download em qualquer status** (produção precisa do arquivo). Item de outro pedido → 404.
- **`OrderItemFileService`**: grava em `<upload-dir>/modelos/<uuid>_<nome-sanitizado>` (sanitização NFD → ASCII `[a-zA-Z0-9._-]`, truncado em 100); caminho relativo em `itens_pedido.arquivo_modelo` (coluna já existia da V6 — **sem migração nova**). Extensões válidas `.stl`/`.3mf` (content types `model/stl`/`model/3mf`); `resolverSeguro` barra path traversal; nome de download = parte após o prefixo UUID. `removerArquivoFisico` é tolerante (loga e segue) e é chamado pelo `OrderService` ao excluir pedido e ao remover itens no PUT.
- `config/StorageProperties` (record, prefixo `application.storage`) criada — primeira consumidora do `upload-dir` do yml. Handler global ganhou `MaxUploadSizeExceededException` → **413** e `MissingServletRequestPartException` → 400.
- **Frontend**: `lib/download.ts` (helper de blob + Content-Disposition, reusado por relatórios), `ordersApi.anexar|baixar|removerArquivoItem`, e controle por item no `OrderFormDialog` (caption + ícones anexar/baixar/remover). **O anexo NÃO invalida a query do pedido**: o caminho fica em estado local `arquivos` (mapa itemId→caminho) porque o refetch dispararia o `reset()` do RHF e descartaria edições não salvas. Itens ainda sem id mostram hint "salve o pedido para poder anexar". Gotcha RHF: `campo.id` do `useFieldArray` é a key interna — o id real do item vem de `watch('itens')[indice].id`.
...
- **`RemoteIpValve` habilitado** (`server.tomcat.remoteip.*` no application.yml): atrás do NGINX todos os clientes chegariam com o IP do container do proxy e compartilhariam um único bucket de rate limit — o valve troca o remote address pelo `X-Forwarded-For` (que o nginx.conf já enviava) quando o peer direto é proxy interno (faixas privadas, caso da rede Docker). Validado ao vivo na stack: 10×401 → 11ª 429 com `Retry-After`, por IP encaminhado; outro IP segue 401.
- Suíte final: **104 testes** (11 novos: 4 unit service + 3 warner + 4 integração 429).

### Evolução pós-roadmap — fluxo automatizado + tema escuro (2026-07-20)
- **Contexto:** com o roadmap de 19 etapas completo, o usuário pediu para simplificar o **uso** do sistema (não remover módulos) — o incômodo real era ter que ir numa tela separada (`Impressões`) e re-digitar impressora/pedido/item/filamento toda vez que uma peça de um pedido `EM_PRODUCAO` ia pra impressão. Decisão: manter todos os módulos, automatizar com uma etapa de confirmação **pré-preenchida** (a escolha da impressora física continua manual — não dá pra automatizar). Também pedido um visual mais moderno: modo escuro por padrão + botões arredondados.
- **`PrintStartDialog`** (`frontend/src/pages/prints/PrintStartDialog.tsx`) ganhou uma prop opcional `predefinido` (`ImpressaoPreDefinida`: pedidoId/itemPedidoId/filamentoId/resumo). Quando presente, os selects de pedido/item/filamento somem (viram um resumo em texto) e só resta escolher a impressora; sem a prop, o comportamento é idêntico ao fluxo avulso original (usado por `PrintsPage`). **`PrintFinishDialog`** ganhou `pesoSugeridoG` opcional, aplicado só no modo `concluir` como valor inicial (ainda editável) do peso consumido — vem do `pesoEstimadoG` do item do pedido.
- **`OrderFormDialog`** (bloco somente-leitura, quando `pedido.status === 'EM_PRODUCAO'`): cada item ganhou uma linha de ação de impressão — uma query própria (`printsApi.listar({status:'EM_ANDAMENTO'})`, filtrada no cliente pelos ids dos itens do pedido) decide se mostra um chip de status + concluir/falhar/cancelar (job já em andamento) ou o botão **"Iniciar impressão"** (abre o `PrintStartDialog` pré-preenchido). Os três diálogos (`PrintStartDialog`, `PrintFinishDialog`, `ConfirmDialog` de cancelar) são renderizados como irmãos do `Dialog` principal do pedido, no mesmo padrão de estado/mutations que `PrintsPage` já usava — a invalidação de `['prints']` feita pelos diálogos filhos cobre a query local por prefixo do TanStack Query.
- **`api/prints.ts`**: `FiltrosImpressoes` ganhou `itemPedidoId` (o backend já aceitava esse filtro desde a Etapa 12; só faltava no client).
- **Tema** (`frontend/src/theme.ts`): `palette.mode: 'dark'` com a mesma família petróleo/verde-azulado dos relatórios PDF (primary/secondary clareados a partir de `#34495e`/`#00897b` para contraste em fundo escuro; tons originais preservados em `.dark`), fundo (`background.default`/`paper`) na mesma família em vez do cinza neutro padrão do MUI, e `shape.borderRadius: 10` — `Button`, `Paper` (Dialog/Card/Menu) e `OutlinedInput` (TextField/Select) já leem esse valor, então uma única propriedade resolve "botões arredondados" de forma consistente sem `styleOverrides` por componente. `main.tsx` ganhou `toastOptions` no `Toaster` (react-hot-toast não herda tema MUI) e `DashboardPage.tsx` ganhou `contentStyle`/`labelStyle`/`itemStyle` nos `Tooltip` do Recharts (o tooltip é chrome de contêiner, não cor de série — ficou branco por padrão e destoava do fundo escuro).
- **Fora de escopo, deixado para depois:** a paleta categórica dos gráficos do Dashboard (`AZUL #3572b0` / `LARANJA #c8611a`, validada com a skill dataviz para fundo claro) não foi revalidada para fundo escuro — só o chrome (tooltip) foi ajustado.
- **Menu lateral reorganizado** (`frontend/src/layout/AppLayout.tsx`, mesmo dia): o usuário trouxe um mockup de referência (dashboard estilo SaaS moderno, gerado por IA) pedindo o menu agrupado por seção. `ITENS_MENU` (lista plana) virou `GRUPOS_MENU` (`{ titulo?, itens[] }[]`) — Dashboard sem seção no topo, depois **Produção** (Pedidos/Impressões/Orçamentos), **Estoque** (Filamentos/Estoque), **Equipamentos** (Impressoras), **Financeiro**, **Clientes**, **Relatórios**, **Configurações** (Usuários). Deliberadamente **não** foram criadas páginas novas que só existem no mockup (Manutenção, Perfis de Impressão, Insumos/Peças e Acessórios/Movimentações como itens separados de Estoque) — o agrupamento usa só módulos reais do sistema. Item ativo passou de borda direita para **pílula arredondada** (`alpha(primary.main, 0.18)` + `borderRadius: 2`, combinando com o `shape.borderRadius` do tema escuro). `gruposVisiveis` filtra por `usuario.role` e descarta grupos que ficam vazios (ex.: Configurações some para quem não é ADMINISTRADOR).

### Redesign do Dashboard + cabeçalho no mesmo estilo do mockup (2026-07-21)
- **Contexto**: o usuário pediu pra "ficar igual à foto" (o mockup SaaS citado acima). Regra seguida: só dado real dos endpoints já existentes — nada de métrica inventada. Onde o mockup mostrava algo sem lastro no backend, foi adaptado ou omitido (documentado abaixo) em vez de fabricado.
- **Tema** (`frontend/src/theme.ts`): `primary` virou índigo/violeta (`#9085e9` main, `#4a3aa7` dark, `#14101f` contrastText) e `secondary` virou verde-água (`#199e70`). Cores validadas com a skill **dataviz** (`validate_palette.js`) contra o fundo escuro do app — inclusive o contraste de texto (branco sobre o violeta claro só dava 3.13:1, por isso o texto dos botões primary é escuro, não branco). Paleta de série dos gráficos (`AZUL`/`LARANJA` em `DashboardPage.tsx`) trocada para os steps já validados em modo escuro (`#3987e5`/`#d95926`) — fecha o "fora de escopo" deixado na etapa anterior.
- **`frontend/src/pages/dashboard/`** (novo): `Sparkline.tsx` (mini `AreaChart` sem eixos, usado nos cards de Pedidos/Faturamento), `AcoesHoje.tsx` (faixa de alertas — pedidos pendentes, impressões em andamento, estoque baixo — só com dado real, sem os "impressões atrasadas/parada" do mockup, que não existem no modelo), `ProducaoAndamento.tsx` (`printsApi.listar({status:'EM_ANDAMENTO'})`, mostra tempo decorrido desde `iniciadoEm` — sem % de progresso fabricado, sem temperatura), `EstoqueCritico.tsx` (mescla `filamentsApi`/`inventoryApi` com `estoqueBaixo:true`), `AtividadeRecente.tsx` (mescla últimos pedidos + últimos clientes por `criadoEm` — é aproximação, não um log de auditoria real), `ResumoFinanceiroDonut.tsx` (`PieChart` com `financialApi.resumo()`).
- **`DashboardPage.tsx`**: `CartaoIndicador` ganhou ícone em badge circular colorido, chip de variação % (só em Pedidos/Faturamento, calculada entre os 2 últimos pontos de `vendasMensais` — os cards sem série mensal não fabricam tendência) e sparkline. "Taxa de sucesso" virou gauge radial (`RadialBarChart`, semicírculo) mantendo o mesmo dado/rótulo de antes. Removidos da grade os 2 mini-cards do mockup sem dado real ("horas de impressão hoje" — endpoint só é mensal; "filamento consumido" isolado — já tem gráfico mensal próprio).
- **Cabeçalho** (`AppLayout.tsx` + `layout/BuscaRapida.tsx` + `layout/NotificacoesMenu.tsx`): saudação dinâmica por hora do dia; **busca rápida funcional** (`Ctrl+K` foca o campo; debounce de `ordersApi`/`clientsApi`/`filamentsApi` por `busca`; clicar num resultado navega pra listagem — não abre o item direto, por escopo); botão **Novo pedido** navega para `/pedidos?novo=1`, e `OrdersPage` lê esse parâmetro (`useSearchParams`) pra abrir o `OrderFormDialog` existente automaticamente (sem duplicar lógica de criação) e limpa o parâmetro da URL; sino de notificações reaproveita a mesma query `['dashboard','resumo']` da página (cache compartilhado) pra badge + menu de alertas reais; avatar virou um `Menu` (Trocar senha / Sair), substituindo os dois ícones soltos de antes. `Fab` de ajuda no canto inferior direito abre um diálogo simples (atalho de busca + link do Swagger) — sem conteúdo fabricado.
- Validado em browser real (scripts `redesign-*`/`check-redesign.mjs` no scratchpad de drives): dashboard completo com dado real, busca com resultado clicável, "Novo pedido" abrindo o diálogo a partir do cabeçalho, sino com os alertas certos, menu do usuário, `Ctrl+K`, tela de login com o novo índigo. `npm run build`/`lint` verdes; sem mudança de backend.
- Validado em browser real (playwright-core + Edge, scripts novos em `scratchpad/browser-drive/`: `drive-impressao-pedido.mjs` — fluxo pré-preenchido ponta a ponta + fluxo avulso intacto — e `drive-tema-escuro.mjs`). `npm run build` e `npm run lint` verdes; sem mudança de backend.
